...
export default class extends Language {
  constructor (store: LanguageStore, file: string[], directory: string, options?: LanguageOptions) {
    ...
    this.language = {
      ...
      PREFIX_REMINDER: (prefix = this.client.user ? this.client.user.toString() : '???') => `このサーバーのプレフィックス: ${Array.isArray(prefix) ? prefix.map(pre => `\`${pre}\``).join(', ') : `\`${prefix}\``}`,
      ...
      COMMAND_INVITE: () => [
        `サーバーに${this.client.user ? this.client.user.toString() : 'ReviewBot'}を追加するには:`,
        `<${this.client.invite}>`,
        util.codeBlock('', [
